...
let playGameBtn = document.getElementById("playerNameForm")
playGameBtn.addEventListener("submit", function(event) {
  player1.name = document.getElementById('player1').value;
  player2.name = document.getElementById('player2').value;
  let playUntil = document.getElementById('gamesTo').value;
...
function playGame(player1, player2, playUntil) {
  ...
  if (p1Counter == playUntil) {
    ...
    console.log("----------");
    console.log();
    return alert(player1.name + " wins with a score of '"+p1Counter+"' to " +player2.name+"'s score of '"+p2Counter+"' !");
  } else if (p2Counter == playUntil) {
    console.log(player2.name + " has reached " + playUntil+" win(s)!");
    console.log("----------");
    console.log();
    return alert(player2.name + " wins with a score of '"+p2Counter+"' to " +player1.name+"'s score of '"+p1Counter+"' !");
  }
}
...
function printToHTML(results) {
  let divElement = document.getElementById('printResults');
  divElement.innerHTML += results + '<br>';
  console.log(results);
}